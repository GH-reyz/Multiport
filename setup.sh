...
cek=$( curl -sS https://raw.githubusercontent.com/GH-reyz/IP/main/access | awk '{print $2}'  | grep $MYIP )
Name=$(curl -sS https://raw.githubusercontent.com/GH-reyz/IP/main/access | grep $MYIP | awk '{print $4}')
if [[ $cek = $MYIP ]]; then
...
BURIQ() {
    curl -sS https://raw.githubusercontent.com/GH-reyz/IP/main/access >/root/tmp
...
PERMISSION() {
    MYIP=$(wget -qO- ipv4.icanhazip.com);
    IZIN=$(curl -sS https://raw.githubusercontent.com/GH-reyz/IP/main/access | awk '{print $2}' | grep $MYIP)
    if [[ "$MYIP" = "$IZIN" ]]; then
        Bloman
    else
...
clear
#echo -e "${red}♦️${NC} ${green}Established By GH-reyz 2022${NC} ${red}♦️${NC}"
...
#wget -q -O /usr/share/nginx/html/index.html "https://raw.githubusercontent.com/GH-reyz/Multiport/main/OTHERS/index.html"
...
echo -e "${RB}      '-------------------------------------------'${NC}"
echo -e "${BB}————————————————————————————————————————————————————————${NC}"
echo -e "      ${WB}Multiport Websocket Autoscript By GH-reyz${NC}"
echo -e "${BB}————————————————————————————————————————————————————————${NC}"
